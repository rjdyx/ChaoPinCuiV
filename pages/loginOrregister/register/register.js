...
Page({
  ...
  validateEmail: function(e){
    ...
    if (!emailreg.test(e.detail.value)){
      app.showToast('请输入正确邮箱', '../../../image/gth.png', 1500)
    }
  },
  // 验证用户名
  ...
  formSubmit: function(e) {
    ...
    if (phone.length == 0 || name.length == 0 || password.length == 0 || email.length == 0 || checkPassword.length == 0) {
      app.showToast('请输入完整信息', '../../../image/gth.png', 1500)
    } else if (password != checkPassword) {
      app.showToast('密码不一样', '../../../image/gth.png', 1500)
    } else if (phone != null && name != null && email != null && password != null && checkPassword != null && this.data.isChecked == false) {
      app.showToast('请勾选协议', '../../../image/gth.png', 1500)
    } else {
      ...
      wx.request({
        ...
        success: function(res) {
          console.log(res)
          if (res.data == 101) {
            app.showToast('用户名存在', '../../../image/gth.png', 1500)
          } else if (res.data == 102) {
            app.showToast('邮箱存在', '../../../image/gth.png', 1500)
          } else if (res.data == 200) {
            app.showToast('注册成功', '../../../image/pass.png', 1500)
          } else if (res.data == 300) {
            app.showToast('微信号已被绑定', '../../../image/gth.png', 1500)
